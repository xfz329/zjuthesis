\chapter{子痫前期及光电容积脉搏波的生理学基础}
\section{引言}
子痫前期会引起孕妇机体一系列复杂的生理、病理反应，会对孕妇患者全身各脏器各系统等造成不良影响。本章首先阐述了子痫前期引起的生理变化及危害。然后介绍脉搏波的产生机理、容积脉搏波血流模型、
以及光电容积脉搏波的检测方法和波形特征，并概述了脉搏波的描述实质。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.7\linewidth]{intro/double}
...
\section{小结}
本章内容对子痫前期可能导致孕妇患者出现的的病生理变化进行了介绍。作为子痫前期的典型症状之一的血管痉挛是可以通过光电容积脉搏波被反应。因此，本章也详细介绍了光电容积脉搏波的生理学基础知识，包括产生原理、采集原理、典型形态特征及信号特征、
常见的生理参数与非生理模型建立的微循环模型。本章的最后也对光电容积脉搏波的量化描述过程进行了类比概括。