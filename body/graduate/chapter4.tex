...
其形态图像越向左上方凸出；而当外周阻力增加时，会导致动脉血管壁弹性下降，下降支波形向上方凸出。描述该过程中的常见面积类参数如\autoref{fig:areafeature}及\autoref{tab:areafeature}所示。
\begin{figure}[htbp]
    \centering
...
\begin{table}[htbp]
    ...
    \midrule
    上升支面积 &       &  $S_r=\int_{T_S}^{T_P}P(t)dt$\\
    下降支面积 &       &  $S_f=\int_{T_P}^{T_E}P(t)dt$\\
    全周期面积 &       &  $S_t=\int_{T_S}^{T_E}P(t)dt$\\
...
值得一提地是，ADR\cite{Feng2018}

此外，罗志昌等人提出了另一种基于脉搏波面积变化的形态学特征量K值\cite{Luo1988,PPGYY}，如\autoref{fig:k}所示。其具体定义为
\begin{equation}
    \label{equ:ppgk}
