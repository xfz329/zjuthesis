...
\begin{equation}
    \label{equ:filter}
    y(n)=\frac{y(n-2)+y(n-1)+y(n)+y(n+1)+y(n+2)}{5}
\end{equation}
经滑动滤波前后的PPG信号对比图如\autoref{fig:filter}所示。信噪比（signal-noise rate，SNR）与均方误差（root mean square error，RMSE）是最常用的评估滤波效果的两个指标，若以$X$表示原始信号，$Y$表示滤波后信号，则有
\begin{equation}
    \label{equ:snr}
...
利用PPG波峰的局部最大值原理进行波形初筛，随后特创新性地引入了PPG波形检测结果的多标准复核与决策确认机制。由于本研究中采集得到的PPG信号不存在运动伪差干扰，采集得到的所有有效PPG波形信号应具有高度相似性，且有效PPG信号与干扰信号、畸变信号在
形态特征、统计特征方面均存在较大差异，存在合理设计相应算法实现自动区分的可行性。因此，本研究设计了系列的PPG波形共性标准，所有初筛“可疑波形”经特定标准复核后均会得到基于该标准的一个输出判定结果。检测算法的决策模块负责对多个标准的复核结果进行
最终决策，将“可疑波形”判断识别为正确波形或错检干扰段。上述复合检测过程如\autoref{fig:detect}所示。
\begin{figure}[htbp]
    \centering
...
只要搜索原始信号得到局部极大值与极小值，再使两者之间满足一定的坐标间隔（即脉搏波相应的间期规则），即可对得到的波峰与波谷点进行初步确认，这一过程如\autoref{alg:detect}所示。
...
计算并遍历原始信号的一阶导数$X^{'}$，若出现$X_i^{'}\ge 0$且$X_{i+1}^{'}\le 0$，则说明出现了极大值。此时，定义时长一大一小的两个搜索窗，分别以当前数据点为窗中心，向前后双向检测并返回窗内最大值的位置。
若两个搜索窗的返回结果一致，则说明返回值就是一个波峰点。结合正常波形的形态特点，本研究将搜索窗时长分别设置为0.1s与0.3s。前者可以直接剔除掉时间跨度过小的局部极值，后者可以保证重博峰点不会被误检为整个波峰。
为防止同一波峰被多次检测，只有与上一成功检测的波峰位置点不同返回值才会被加入波峰缓存数组$Peaks$中。

在实际检测时，常会出现一阶导数在某邻域内多次出现过零点，导致多个极大极小值连续出现。1 时长 2 对称 3 缓存而由于研究采用了中心对称搜索窗，实际当第一个过零点出现时，这一邻域内的所有极值点均已被检索过。此外，为提高进一步搜索窗返回窗内最大值位置的效率，
上一次搜索的返回值会被缓存，由于窗口单向时移滑动的特点，
...
对正常PPG波形，上述检测算法可以实现较好的检测效果，但对干扰信号的检测能力不足、对畸变信号的识别判断能力有限。在前期调试时，上述检测算法极易对类似\autoref{fig:samplesignal}中的无效数据段信号进行识别与错误检测。
因此，波形检测算法引入了二次评估与最终决策机制以减少此类错检发生的概率。

1.评估标准

从被试采集得到的脉搏波信号，其有效PPG波形应具有一定的相似性，在形态特征与统计特征上应高度贴近。基于此信息，本研究设计了多个使得具有且仅具有PPG波形一般特征的初筛“可疑波峰”可以通过的评估标准，
用以识别区分PPG波形与干扰畸变信号。设脉搏波的相对幅值$A$是采样时间$t$的函数，将其表示为$A(t)$。

\Rnum{1}.能量与功率标准

参考信号与系统中对时间信号能量的定义\cite{Alan2019}，则单个PPG信号的能量可表示为
\begin{equation}
    \label{equ:ppge}
...
此外，由于波形检测算法中已经设定了最小间期与最大间期分别为111与222。若参照心率定义一分钟中内有效脉搏波的数量为脉率(Pulse Rate，PR)。那么，原则上若待检波形的PR

则理论上无法检测。 在实际检测过程中可视具体信号对相应时间参数进行调整。
