...
高阶统计量（High Order Statitics，HOS）是指非传统的二阶统计量（如时域相关函数、频域功率谱密度等）的三阶或三阶以上统计量\cite{Zhang2002,Tu2013}。高阶统计量可识别非最小相位系统，对加性噪声的抗干扰能力强。
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\linewidth}{cX<{\centering}c}
        ...
        脉冲因子&信号峰值与信号整流值（绝对值的平均值）的比值&$I=\frac{X_{peak}}{|\mathop{X}\limits^-|}$      \\
        裕度因子&信号峰值与信号方根幅值的比值&$C_e=\frac{X_{peak}}{X_R}$       \\
        波形因子&信号有效值与信号整流值的比值&$S_f=\frac{X_{RMS}}{|\mathop{X}\limits^-|}$      \\
...
以上参数的具体定义如\autoref{tab:ndp}所示。由数字信号处理的原理可知，加权计算均值可以看成是一低通滤波模型，对脉冲信号有较好的抑制作用。可以发现，峰值因子、脉冲因子、裕度因子具有相似的物理定义，均是信号峰值与某一统计量的均值进行比较，
因而，对脉冲故障比较敏感。
而波形因子是两个统计量均值的比值，对脉冲信号的敏感性不高。另一方面，高阶统计量峭度因子和偏度反应的是原始信号的分布特性，如\autoref{fig:skew}所示。峭度因子常在信号处理中作为高斯信号与非高斯信号的划分。高斯信号的峭度因子为3。
偏度主要反应信号的左右对称性，完全对称的信号其偏度为0。
\begin{figure}[htbp]
    \centering
