...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.6\linewidth]{ch4/areafeature}
...
早在上个世纪八十年代，罗志昌等人就提出了一种基于脉搏波面积变化的形态学特征无量纲参数K值\cite{Luo1988,PPGYY}，其具体定义为
\begin{equation}
    \label{equ:ppgk}
...
同时能够在很大程度上反应血管外周阻力与血管壁硬化程度等生理因素的变化，在临床中得到了普遍应用。考察\autoref{tab:areafeature}中全周期面积比与K值的定义，同时结合相关数学知识，不难
推导得到
\begin{equation}
    \label{equ:kandart}
...
特征描述向量是本研究为更准确描述PPG波形形态而提出一类特征。

由于特征描述向量的维数$n$可以在设计时人为指定，上小节提到的如等多项脉搏波时域参数即可视为$n=1$的描述向量；若设定原始信号采样点数为$N$，则PPG原始数据可以视为$n=N$的描述向量；
预处理中对PPG信号进行插值处理后的数据可以视为$n>N$的超维描述向量。则显然随着向量维数的增加，对原始脉搏波的描述能力越强。这一现象可以用信息论中的编码理论进行解释，即通讯过程中增加编码长度可以提高系统的信息传输能力。
...
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=.6\linewidth]{ch4/pow}
...
\end{equation}
本研究在实际计算时选取了$f(x)=x^2$与$f(x)=x^{\frac{1}{2}}$两个基底幂函数进行计算，并将最后的EDI以向量形式记为
\begin{equation}
    \label{equ:edi}
