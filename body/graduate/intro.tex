...
\begin{figure}[htbp]
    ...
    \caption[子痫风险因子的聚类分析热力图示意]{\label{fig:Heatmap}子痫风险因子的聚类分析热力图示意\cite{Villa2017}。其中，横轴为各风险因子，纵轴为不同的簇；左图代表了聚类的效果，黑色单元格的大小说明了簇的大小，右图单元格的颜色代表了相应簇中相应结果的风险检验结果。}
\end{figure}
...
第七章是低耦合高拓展的软件综合分析系统的设计与实现。本章将本研究的各项工作按云+端+智能的形式进行了软件设计，阐述了包括数据源导入管理、预处理算法管理、特征拓展管理、标准数据管理、模型训练生成管理等
方面在内的整体分析系统的软件设计思路，介绍了为提高本研究各部分内的内聚性、降低各研究内容之间的耦合性、提高系统整体可拓展性及实用性、提高系统真正落地临床的可能性所做出的各项规划及实现。

第八章是总结与展望。对本论文的全部研究工作进行系统性总结，阐述本论文的创新工作点，并对下一阶段的研究工作内容进行了规划与展望。