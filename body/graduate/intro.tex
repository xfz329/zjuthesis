...
由于PE是妊娠期高血压疾病的一种，BP一直是临床用于PE监测、诊断的重要指标之一\cite{OAG9,HDASOM,2000s1}。在进行BP测量时，孕妇同一手臂应至少测量两次，若两次的测量结果的SBP超过$90mmHg$和（或）DBP超过$90mmHg$，则将该孕妇诊断为高血压；
...
Leona C.Y. Poon等人对5590名单胎孕妇的一项研究表明，单独使用MAP检测PE的检出率为38\%；而将孕妇病史等风险因子与MAP结合使用时，PE的检出率可达63\%，假阳性率为10\%\cite{Poon2008}。
Stamilio等人发现，孕妇在初次产前检查发现MAP大于$90mmHg$与其PE病发的相关性显著\cite{Stamilio2000}。
...
2019年，Duhig等人\cite{Duhig2019}对1019名疑似PE的孕妇进行了追踪实验，在整个实验期间一直持续性地检测孕妇的PLGF水平。结果显示，针对实验组（n = 573），PE诊断时间的中位数为4.1天，而对照组（n = 446）这一数值仅为1.9天，这证明PLGF可以有效的缩短PE确诊的时间。
...
VICTOR2系列及DELFIA Xpress系列的多款免疫荧光分析仪平台，可应用于多种孕期综合并发症的临床检测之中，同时可对包括AFP、hCG、PAPP-A、PlGF、
sFlt-1、uE3等在内多种PE生物标志物进行检测标定，如\autoref{fig:PerkinElmer}所示。与此同时，该公司也配套研发了LifeCycle软件分析系统，可实现了从样品接收、检验检测、风险评估及产出综合报告的全自动分析工作流程。
\begin{figure}[h]
    \centering
...
\subsection{分析技术}
由于现代化医学检测设备的普及及医学信息化技术的发展，医学检查过程中的各项数据激增；另一方面，激增的数据也促进了计算机领域机器学习（Machine learning, ML）的不断创新发展。
在此背景下，近年来相关研究者也开始将ML相关技术与算法应用于PE的识别、判断及预测等问题的研究中。

\autoref{tab:AIinPE}总结了近年来使用ML技术进行PE相关问题的多项研究。其中，数据集规模一栏标注了这些研究涉及的总实验人数或实验组与对照组人数。从\autoref{tab:AIinPE}不难发现，
利用ML技术分析前小节提到的PE各项风险因子、生物标志物等参数是目前炽手可热的多学科交叉研究点，但利用人体电生理参数进行分析处理的研究目前还较为稀少，如\autoref{tab:AIinPE}突出显示的部分。
更进一步，从具体使用的ML技术与算法的角度，可以将\autoref{tab:AIinPE}中的诸多研究分为数据挖掘与分类聚类分析等两类应用研究方向\cite{Mehta2016}。

一、数据挖掘

数据挖掘，又称为关联规则，力求发现多项数据之间的隐藏的逻辑与关系\cite{Han2006}。在PE的相关研究中，利用数据挖掘技术从血浆代谢物、蛋白质及RNA等生化指标中筛选出与PE病发相关性较强的成份一直是此类研究的热点。
而这些研究往往都是在实现数据降维的同时揭示数据之间隐藏的联系。

2005年，Louise C. Kenny等人\cite{Kenny2005}从利用基因遗传算法（genetic programming, GP），通过血浆中特定代谢物成份来识别PE病发的孕妇。他们对受试孕妇的血浆成份进行了分析（其中，PE病发的实验组人数为$N_{PE}=87$，正常对照组人数为$N_{Normal}=87$，下同）
，通过GP算法最后训练得到了仅使用三种代谢物峰值变量的PE预测模型。结果显示，该模型可以较好的区分PE病发孕妇与正常孕妇，其灵敏度高达100\%，特异性高达98\%。

2018年，Liron Yoffe等人\cite{Yoffe2018}为寻找能有效区分识别PE的转录RNA，对孕妇妊娠前三个月的血浆中的非编码循环RNA的丰度进行了分析。在对孕妇的非编码RNA测序后，他们确定了实验组与对照组（$N_{PE}=75$，$N_{Normal}=75$）
之间差异表达的25个RNA，并基于该结果训练生成了一个逻辑回归预测模型。他们的研究结果表明，该模型
\begin{landscape}
    \zihao{-5}
...
2018年，Muhlis Tahir等人\cite{Tahir2018,Tahir2018-2}对比了神经网络（Neural Networks, NN）和深度学习（deep learning, DL）等两类算法在预测妊娠期孕妇PE病发的风险水平这一问题上的结果。
他们使用粒子群优化（Particle swarm optimization, PSO）作为特征选择算法，将原始数据集（$N=1077$）的17个参数降维至9个。
在原始数据上通过留一法（Leave One Out，LOO）验证表明，深度学习后的模型具有95.12\%的准确率，使用经过缩减的数据集准确性还可提升至95.68\%。而当使用神经网络算法时，算法准确性亦可高达96.66\%。

2020年，Jose F Carre˜no等人\cite{Carreno2020}基于蛋白质组学数据，改进了基于比较的特征选择算法进行了PE的预测。特征算法包括帝国竞争与基因集簇两种降维算法与三点时间序列（分别对应孕前中晚期数据）总结算法两方面。
