...
\begin{figure}[htbp]
    ...
    \caption{\label{fig:PerkinElmer}美国PerkinElmer公司的两款检测设备}
\end{figure}
\vspace{1cm}
...
2021年，Yixin Li等\cite{LI2021102}通过逻辑回归、随机森林、支持向量机和极限梯度提升（XGBoost）等算法对孕早期和孕中期孕妇的电子病历进行了分析，从38个临床参数中构建了PE的识别模型（$N$=3759） 。
其研究结果表明，由XGBoost算法生成的模型具有最佳的预测性能，其准确率为0.920，精确度为0.447，AUC数值为0.955。

2023年，Hao Wang等\cite{HW2023}设计了一种计算生物学方法，从单细胞转录组（scRNA-seq）来识别病理细胞亚群并预测孕妇的PE病发风险（$N_{PE}$=7970，$N_{Control}$=7178）。
他们的结果表明，将极限梯度提升（XGBoost）算法与ReliefF特征选择算法结合使用时，可以取得最优的识别模型性能。通过最佳模型对健康胎盘的九个细胞亚群进行分类，可达到92.61\%的准确率与92.46\%的召回率。
...
2017年，Pia M. Villa等\cite{Villa2017}通过贝叶斯聚类算法，通过PE风险因子的数据对受试孕妇进行了聚类分析（$N=903$）。他们还计算了聚类分析得到的各簇的PE病发可能。
结果表明，PE的患发可能随孕妇具有的PE风险因子数量增加而呈指数增长。同时，PE病发的程度也往往随孕妇具有的风险因子的种类差异而呈现不同。

2020年，Ivana Mari{\'{c}}等\cite{Maric2020}利用弹性网（elastic net，EN）算法与梯度提升（gradient boosting，GB）算法从67项PE的风险因子等中训练了PE预测模型与早发PE预测模型（$N=16 370$）。经验证，PE预测模型的AUC数值可达0.79，识别假阳性率为8.1\%；早发PE预测模型的
...
2022年，Leon J. Schmidt等\cite{SCHMIDT202277}分析了孕妇的sFlt-1、PlGF与多项超声检测数据（包括脐动脉脉动指数、大脑中动脉脉动指数、平均子宫动脉脉动指数等），在此基础上构建了114个特征。随后，他们
使用了梯度增强树（gradient-boosted tree）与随机森林算法进行了PE识别模型的构建。结果表明，梯度增强树模型的阳性预测值为88\%±6\%，阴性预测值为89\%±3\%，灵敏度为66\%±5\%，特异性为97\%±2\%，总体准确度为89\%±3\%；
而随机森林模型的阳性预测值为88\%±6\%，特异性为97\%±1\%，其他指标表现稍差。
...
2008年，KARLIJN VOLLEBREGT等\cite{KARLIJN2008}研究了使用脉搏波数据进行PE初筛的可能。通过对健康孕妇（$N=223$）的跟踪研究表明舒张压、心输出量与脉搏波增强指数均对PE预测模型有明显影响。结合PE风险因子，最终得到的模型
ROC数值可达0.95,其中敏感性90\%、特异性86\%。

2012年，Kathleen Tomsin等\cite{Tomsin2012}对静脉脉搏波传播速度（Pulse Wave Velocity，PWV）进行了研究。研究结果显示在正常妊娠期孕妇所有脏器的PWV均呈现逐渐升高的趋势。
...
2014年，Su Fangming等\cite{Su2014}的研究发现，可以通过光电容积脉搏波无创评估整个妊娠过程中心血管活动情况。他们提取分析了脉搏波增强指数（AIX）、反射指数（RI）、传导时间（PTT）等，
...
2018年，Tammy Y. Euliano等\cite{Euliano2018}通过无创监测心电图与光电脉搏图对识别PE患者（$N_{PE}=25$，$N_{Normal}=31$）进行了研究。他们发现PE患者的脉搏波波峰时长较正常孕妇长、弹性系数较正常孕妇小。
