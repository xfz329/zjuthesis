...
使用$N$=5的平滑滤波器对采集得到的PPG进行处理，滤波前后对比如\autoref{fig:filter}所示。由\autoref{equ:malpf}可知，此时滤波器对应的的截止频率约为89$Hz$。
...
\begin{center}
    ...
    \begin{longtable}{m{3cm}<{\centering}m{9cm}<{\centering}m{2cm}<{\centering}}
        ...
        \colorrowc 外周阻力系数      &  降中峡幅值与主波峰幅值之比         &  $\displaystyle \frac{DD'}{OP}$\\
    \end{longtable}
\end{center}
\vspace{-0.8cm}
...
\begin{center}
	...
	\begin{longtable}{m{3.5cm}<{\centering}m{6.5cm}<{\centering}m{4.5cm}<{\centering}}
        ...
        \bottomline
        \endlastfoot
        \colorrowa 上升支面积      &  从波形起点至峰值点对波形曲线积分所得面积值         &  $\displaystyle S_r=\int_{T_S}^{T_P}P(t)dt$\\
        \colorrowc 下降支面积      &  从波形峰值点至终点对波形曲线积分所得面积值         &  $\scriptstyle S_f=\int_{T_P}^{T_E}P(t)dt$\\
        \colorrowa 全周期面积      &  脉搏波波形在一个完整周期内的积分所得面积值         &  $\scriptscriptstyle S_t=\int_{T_S}^{T_E}P(t)dt$\\
        \colorrowc 上升支面积比    &  上升支面积与$\triangle OPS$面积之比         &   $\textstyle R_r=\frac{S_r}{S_{\triangle OPS}}$    \\
