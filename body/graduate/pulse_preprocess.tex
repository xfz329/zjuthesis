...
\section{引言}
本介绍使用的PPG数据来源自自主进行的临床实验，本章从实验设备、实验流程与数据导出等方面对其进行介绍。
利用常用的变量分析方法，对临床实验被试的人口统计学特征进行统计分析。
介绍PPG的预处理分析中的信号滤波、波形检测、切迹检测、基线漂移处理、信号重采样与数据标准化等步骤。
...
本实验已通过浙江大学医学院附属妇产科医院医学伦理委员会的审查（审查编号：20170131），所有被试孕妇均知晓研究目的及具体实验流程，并签署了知情同意书。

\subsection{实验设备}
本研究对PPG数据采集仪器的专业性、可靠性有较高要求，最终选择了美国GE公司生产的GE Healthcare CARESCAPE B650型麻醉监护仪进行数据采集，如\autoref{fig:monitor}所示。
...
此外，本研究选择了美国泰科公司旗下的Nellcor DS-100A型透射式血氧传感器作为B650型的配套组件，如\autoref{fig:ds100a}所示。
该传感器一般佩戴在被试左手食指上，若被试的左手食指有损伤，则将测量部位替换为左手中指。
在测量时，手指需放入传感器指套内部，使指甲与传感器表面有指甲标记的部位正对，指尖触及但不超出指套顶端，以确保发光管发出的所有光线全部通过被试的组织。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{pulse_preprocess/ds100a}
    \caption{\label{fig:ds100a}Nellcor DS-100A型血氧传感器}
\end{figure}

\subsection{实验流程}
被试到达实验场地后，由实验人员登记其PE风险因子信息，包括姓名、年龄、孕产史等。接着实验人员会告知被试本研究的研究目的与具体实验流程。
...
\begin{center}
    ...
    \begin{longtable}{m{1cm}<{\centering}m{1.8cm}<{\centering}m{2cm}<{\centering}m{4.5cm}<{\centering}m{4.5cm}<{\centering}}
		...
		\topline
        \colorhead \textbf{序号} & \textbf{样本数目}&\textbf{样本相关性}&\textbf{检验方法}&\textbf{检验方法英文名}\\
        \midline
        \endfirsthead
        \caption[]{(续)}\\
        \topline
        ...
        \bottomline
        \endlastfoot
        \colorrowa  1  & 单样本   & /     & 卡方检验  & Chi-Squared Test \\
        \colorrowc  2  & 单样本   & /     & 二项分布检验 & Binomial Test \\
        \colorrowa  3  & 单样本   & /     & K-S检验 & Kolmogorov–Smirnov Test \\
        \colorrowc  4  & 单样本   & /     & 符号秩检验 & Wilcoxon Signed-Rank Test \\
        \colorrowa  5  & 单样本   & /     & 游程检验  & Wald–Wolfowitz runs Test \\
        \colorrowc  6  & 两样本   & 独立    & Wilcxon W等级和检验 & Mann-Whitney U Test \\
        \colorrowa  7  & 两样本   & 独立    & 摩西极端反映差异检验 & Moses Extreme Reaction Test \\
        \colorrowc  8  & 两样本   & 独立    & K-S检验 & Kolmogorov–Smirnov Test \\
        \colorrowa  9  & 两样本   & 独立    & 游程检验  & Wald–Wolfowitz runs Test \\
        \colorrowc  10  & 两样本   & 相关    & 符号检验  & Sign Test \\
        \colorrowa  11  & 两样本   & 相关    & 符号秩检验 & Wilcoxon Signed-Rank Test \\
        \colorrowc  12  & 两样本   & 相关    & 变化显著性检验 & McNemar's Test \\
        \colorrowa  13  & 两样本   & 相关    & 边缘一致性检验 & Marginal Homogeneity Test \\
        \colorrowc  14  & 多样本   & 独立    & K-W平均秩检验 & Kruskal-Wallis H Test \\
        \colorrowa  15  & 多样本   & 独立    & 中位数检验 & Median Test \\
        \colorrowc  16  & 多样本   & 独立    & 分组分布检验 & Jonckheere-Terpstra Test \\
        \colorrowa  17  & 多样本   & 相关    & 双向等级方差分析 & Friedman Test \\
        \colorrowc  18  & 多样本   & 相关    & 肯德尔和谐系数检验 & Kendall's W Test \\
        \colorrowa  19  & 多样本   & 相关    & 二分变量检验 & Cochran's Q Test \\
    \end{longtable}
\end{center}
\vspace{-0.8cm}
...
\begin{center}
    ...
    \begin{longtable}{m{1cm}<{\centering}m{3.5cm}<{\centering}m{3.5cm}<{\centering}m{3.5cm}<{\centering}m{2cm}<{\centering}}
		...
		\topline
        \colorhead \textbf{序号} & \textbf{检验变量（单位）}      & \textbf{实验组（n=44）} & \textbf{对照组（n=35）} & \textbf{$p$值} \\
        \midline
        \endfirsthead
        \caption[]{(续)}\\
        \topline
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & 年龄（years） & 32.3±3.6 & 33.8±4.6 & 0.108 \\
        \colorrowc 2 & 孕周（weeks） & 32.7±3.8 & 34.3±4.3 & 0.053 \\
        \colorrowa 3 & 身高（cm） & 158.1±5.0 & 160.0±3.3 & 0.089 \\
        \colorrowb \colorcellc 4 & \textbf{体重（kg）} &  \textbf{75.7±12.9} &  \textbf{67.1±8.2} & \textbf{0.002* }\\
        \colorrowb \colorcella 5 & \textbf{BMI（kg/cm）} &  \textbf{30.2±4.5} &  \textbf{26.2±3.3} & \textbf{<0.001*} \\
        \colorrowb \colorcellc 6 & \textbf{收缩压（mmhg）} &  \textbf{160.1±19.5} &  \textbf{111.2±9.8} & \textbf{<0.001*} \\
        \colorrowb \colorcella 7 & \textbf{舒张压（mmhg）} &  \textbf{96.1±14.5} &  \textbf{66.8±10.4} & \textbf{<0.001*} \\
        \colorrowc 8 & 心率（bpm） & 87.0±11.9 & 87.5±13.1 & 0.656 \\
    \end{longtable}
\end{center}
\vspace{-1.5cm}
...
\begin{figure}[htbp]
    ...
    \caption{\label{fig:samplesignal}一名被试实采PPG信号示意图}
\end{figure}
...
使用$N$=5的平滑滤波器对采集得到的PPG进行处理，滤波前后对比如\autoref{fig:filter}所示。由\autoref{equ:malpf}可知，此时滤波器对应的的截止频率约为89Hz。
...
\begin{center}
    ...
    \begin{longtable}{m{1cm}<{\centering}m{1.5cm}<{\centering}m{3.6cm}<{\centering}m{3.6cm}<{\centering}m{3cm}<{\centering}}
		...
		\label{tab:checkingp}\\
		\topline
        \colorhead \textbf{序号} & \textbf{复核标准}      & \textbf{个体全部波形参与均值计算百分比区间} & \textbf{判为有效波形的均值相对区间} & \textbf{备注} \\
        \midline
        \endfirsthead
        \caption[]{(续)}\\
        \topline
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & $P$ & [0\%, 100\%] & [0.4, 1.8] & 无需排序 \\
        \colorrowc 2 & $S$ & [0\%, 100\%] & [0.4, 1.8] & 无需排序 \\
        \colorrowa 3 & $R$ & [20\%, 80\%] & [0.8, 1.2] & 需按升序排序 \\
        \colorrowc 4 & $B_1$ & [0\%, 80\%] & [0, 8] & 需按升序排序 \\
        \colorrowa 5 & $B_2$ & [0\%, 80\%] & [0, 10] & 需按升序排序 \\
        \colorrowc 6 & $B_3$ & / & [0.5, 2.0] & 无需计算均值 \\
    \end{longtable}
\end{center}
\vspace{-1cm}
...
\end{figure}
\noindent
形均在\autoref{fig:scd_detect1}与\autoref{fig:scd_detect2}中用虚线进行了框选，其中黑色虚线表示两种算法对PPG波形的识别判断完全一致，红色虚线表示两种算法的检测结果出现了分歧。
...
\begin{center}
    \zihao{-5}
    \begin{longtable}{m{1cm}<{\centering}m{2cm}<{\centering}m{1.5cm}<{\centering}m{1.5cm}<{\centering}m{1cm}<{\centering}m{1.5cm}<{\centering}m{1cm}<{\centering}m{1.5cm}<{\centering}m{1cm}<{\centering}}
        ...
        \colorhead \multirow{-2}{*}{\textbf{序号}} & \multirow{-2}{*}{\textbf{数据源}}      & \multirow{-2}{*}{\textbf{波形总数/个}} & \textbf{错检数/个}      & \textbf{准确率} & \textbf{错检数/个}      & \textbf{准确率} & \textbf{错检数/个}      & \textbf{准确率} \\
        \endfirsthead
        \caption[]{(续)}\\
        \topline
        \colorhead    &  &  & \multicolumn{2}{c}{\textbf{SCD算法}} & \multicolumn{2}{c}{\textbf{DMTW算法}} & \multicolumn{2}{c}{\textbf{HeartPy算法}}  \\
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & 自主实验数据 & 7864 & \colorcellb \textbf{26} &  \colorcellb \textbf{99.6\%} & 112 & 98.6\% & 97 & 98.7\% \\
        \colorrowc 2 & CBPEDS & 17562 &  \colorcellb \textbf{50} &  \colorcellb \textbf{99.7\%} & 182 & 99.0\% & 168 & 99.0\% \\
    \end{longtable}
\end{center}
\vspace{-0.8cm}
...
\begin{figure}[htbp]
    \centering
    \subfigure[重博波明显的PPG波形示意图]{
    \includegraphics[width=5.5cm]{pulse_preprocess/ri1}
    }
    \quad
    \subfigure[重博波不明显的PPG波形示意图]{
...
\begin{figure}[htbp]
    ...
    \caption{\label{fig:drift}基线漂移去除原理示意及处理前后对比图}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{\label{fig:spline}经过4点的线性样条插值与三次样条插值对比图}
\end{figure}
...
\begin{center}
    ...
    \begin{longtable}{m{1cm}<{\centering}m{5cm}<{\centering}m{7cm}<{\centering}}
		...
		\topline
        \colorhead \textbf{序号} & \textbf{样条种类}&\textbf{端点条件}\\
        \midline
        \endfirsthead
        \caption[]{(续)}\\
        \topline
...
\begin{figure}[htbp]
    \centering
    \subfigure[\label{fig:ri1}重博波明显的PPG波形示意图]{
    \includegraphics[width=6cm]{pulse_preprocess/ri1}
    }
    \quad
    \subfigure[\label{fig:ri2}重博波不明显的PPG波形示意图]{
...
\begin{center}
    \zihao{-5}
    \begin{longtable}{m{1cm}<{\centering}m{4cm}<{\centering}m{8cm}<{\centering}m{2cm}<{\centering}}
		\caption{常见的PPG时间类参数定义}\\
		\label{tab:timefeature}\\
		\topline
        \colorhead \textbf{序号} & \textbf{参数} & \textbf{物理意义} & \textbf{表达式} \\
        \midline
        \endfirsthead
        \caption[]{(续)}\\
        \topline
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & 脉动周期      &  相邻两个波谷之间的时间间隔         &  $SE$\\
        \colorrowc 2 & 上升支间期      &  从波形起点（波谷）至主波峰之间的时间间隔         &  $SO$\\
        \colorrowa 3 & 下降支间期      &  从主波峰至波形终点（波谷）之间的时间间隔        &  $OE$\\
        \colorrowc 4 & 动脉高压力持续时间    &  主波维持在峰值幅值2/3高度的时间间隔         &    $A'B'$   \\
        \colorrowa 5 & 血管硬度指数    &  动脉高压力持续时间与脉动周期的比值         &   $\displaystyle \frac{A'B'}{SE}$    \\
        \colorrowc 6 & 心肌收缩系数    &  上升支间期与脉动周期的比值         &  $\displaystyle \frac{SO}{SE}$    \\
        \colorrowa 7 & 心搏出系数      &   从波峰至降中峡的时间间隔与脉动周期的比值       &   $\displaystyle \frac{OB'}{SE}$\\
        \colorrowc 8 & 上升支最大斜率间期      &   上升支波谷与上升支斜率最大点之间的时间间隔      &   /    \\
        \colorrowa 9 & 下降支最大斜率间期      &   下降支斜率最小点与下降支波谷之间的时间间隔      &    /  \\
    \end{longtable}
\end{center}
\vspace{-0.8cm}
...
\begin{center}
    \zihao{-5}
    \begin{longtable}{m{1cm}<{\centering}m{3cm}<{\centering}m{7cm}<{\centering}m{2cm}<{\centering}}
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & 波峰幅值      &  主波峰幅值         &  $OP$\\
        \colorrowc 2 & 波谷幅值差值      &  相邻波谷之间的幅值之差         &  /\\
        \colorrowa 3 & 降中峡幅值      &  降中峡至基线幅值         &  $DD'$\\
        \colorrowc 4 & 外周阻力系数      &  降中峡幅值与主波峰幅值之比         &  $\displaystyle \frac{DD'}{OP}$\\
    \end{longtable}
\end{center}
...
\begin{figure}[htbp]
    ...
    \caption{\label{fig:k}K值的计算原理示意图}
\end{figure}
\vspace{-1em}
...
\begin{center}
    \zihao{-5}
	\begin{longtable}{m{1cm}<{\centering}m{3cm}<{\centering}m{6.5cm}<{\centering}m{4.5cm}<{\centering}}
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & 上升支面积      &  从波形起点至峰值点对波形曲线积分所得面积         &  $\displaystyle S_r=\int_{T_S}^{T_P}P(t)dt$\\
        \colorrowc 2 & 下降支面积      &  从波形峰值点至终点对波形曲线积分所得面积         &  $\displaystyle S_f=\int_{T_P}^{T_E}P(t)dt$\\
        \colorrowa 3 & 全周期面积      &  脉搏波波形在一个完整周期内的积分所得面积         &  $\displaystyle S_t=\int_{T_S}^{T_E}P(t)dt$\\
        \colorrowc 4 & 上升支面积比    &  上升支面积与$\triangle OPS$面积之比         &   $\displaystyle R_r=\frac{S_r}{S_{\triangle OPS}}$    \\
        \colorrowa 5 & 下降支面积比    &  下降支面积与$\triangle OPE$面积之比        &   $\displaystyle R_f=\frac{S_f}{S_{\triangle OPE}}$    \\
        \colorrowc 6 & 全周期面积比    &  全周期面积与$\triangle SPE$面积之比         &   $\displaystyle R_t=\frac{S_t}{S_{\triangle SPE}}$    \\
        \colorrowa 7 & 下降支面积差值比&  \tabincell{c}{下降支面积和$\triangle OPE$面积之差\\ 与$\triangle OPE$面积之比}      &    $\displaystyle \Delta R_f=\frac{S_{\triangle OPE}-S_f}{S_{\triangle OPE}}=1-R_f$\\
	\end{longtable}
\end{center}
\vspace{-0.8cm}
...
\begin{center}
    \zihao{-5}
    \begin{longtable}{m{1cm}<{\centering}m{3cm}<{\centering}m{6.5cm}<{\centering}m{2cm}<{\centering}}
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & 上升支平均斜率      &  主波峰值与上升支时间之比         &  $\displaystyle \frac{OP}{SO}$\\
        \colorrowc 2 & 下降支平均斜率      &  主波峰值与下降支时间之比         &  $\displaystyle -\frac{OP}{OE}$\\
        \colorrowa 3 & 上升支最大斜率      &  上升支一阶导数的最大值        &  /\\
        \colorrowc 4 & 下降支最小斜率      &  下升支一阶导数的最小值         &   /    \\
    \end{longtable}
\end{center}
\vspace{-0.8cm}
