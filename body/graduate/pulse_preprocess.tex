...
\section{引言}
本研究使用的PPG数据来源自自主进行的临床实验，本章从研究对象筛选、实验设备、实验场地、实验流程与数据导出等方面对其进行介绍。
利用常用的统计分析方法，对临床实验被试的人口统计学特征进行分析。
进行PPG的预处理涉及的信号滤波、波形检测、切迹检测、基线漂移处理、样条插值与数据标准化等过程的处理与研究。
...
\subsection{研究对象筛选}
在浙江大学医学院附属妇产科医院，随机选择孕妇进行数据采集。

病例纳入标准为：美国麻醉协会（American Society of Anesthesiologists，ASA）Ⅱ 级；
年龄18-40岁的PE患病孕妇；在两次间隔4h或4h以上的血压测定中，收缩压测量≥140 mmHg，和（或）舒张压≥90 mmHg；
24h尿蛋白总量≥300mg或≥1+；孕周≥20w。任一纳入标准不符，此孕妇无法参与研究。

病例排除标准为：椎管内麻醉禁忌；产程活跃期；前置胎盘或其他胎盘异常；多胎妊娠；肺水肿；脐带脱垂；慢性高血压；
拒绝签署知情同意书。任一排除标准符合，此孕妇无法参与研究。

\subsection{实验设备}
本研究对PPG数据采集仪器的专业性、可靠性有较高要求，最终选择的硬件设备如\autoref{fig:hd_devices}所示。
其中，选用了美国GE公司生产的Healthcare CARESCAPE B650型麻醉监护仪进行数据采集，如图\autoref{fig:monitor}所示；
选择了美国Tyco公司的Nellcor DS-100A型透射式血氧传感器作为B650型的配套组件，如图\autoref{fig:ds100a}所示。
...
\begin{figure}[htbp]
    \centering
    \subfigure[\label{fig:monitor}B650监护仪及其配件]{
    \includegraphics[width=7cm]{pulse_preprocess/monitor1}
    }
    \quad
    \subfigure[\label{fig:ds100a}DS-100A型血氧传感器]{
    \includegraphics[width=5.5cm]{pulse_preprocess/ds100a}
    }
    \caption{\label{fig:hd_devices}数据实验选用的硬件设备}
\end{figure}

\subsection{实验场地}
实验场地设置在浙江大学医学院附属妇产科医院的产科检查室。场地内保持正常室内照明灯光，预先布置好检测用床，供被试实验期间以舒适的姿势平躺并安放好手臂。
实验设备由额外的的装置固定安放在床边。

\subsection{实验流程}
实验开始前，实验人员会对实验场地及设备进行必要的消毒处理。
被试到达实验场地后，首先会有实验人员告知其实验的研究目的与具体实验流程。
在征得被试同意后，实验人员会为被试登记姓名、年龄、孕产史等基本信息，相关PE风险因子信息也会一并记录。
随后，实验人员会协助被试躺在检测用床上，并按操作规范为被试左手食指佩戴血氧采集探头，若被试的左手食指有损伤，则将测量部位替换为左手中指。
被试手指需放入传感器指套内部，使指甲与传感器表面有指甲标记的部位正对，指尖触及但不超出指套顶端，以确保发光管发出的所有光线全部通过被试的组织。
被试需要保持平卧姿势，禁止说话发声或产生较大的肢体动作。
在被试休息至少5min后，由实验人员开始PPG数据采集，单次采集时长不少于1min。
...
剩余79例为有效数据，包括44例实验组数据和35例对照组数据。\autoref{fig:pe_healthy}给出了实验采集得到的健康孕妇与子痫患者的脉搏波波形对比，
从中可以发现两者的幅值大小、周期长短及重搏波明显程度等方面均存在一定的差异。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=.6\linewidth]{pulse_preprocess/pe_healthy}
    \caption{\label{fig:pe_healthy}实验采集得到的健康孕妇与子痫患者的脉搏波波形对比}
\end{figure}
...
\begin{longtblr}
    ...
    ]
    {
        ...
        row{1}         = {font=\headfont},
        row{2-Z}       = {font=\nonheadfont},
        cell{2}{2}     = {r=5,c=1}{c,m},
        cell{7}{2}     = {r=5,c=1}{c,m},
        cell{12}{2}    = {r=3,c=1}{c,m},
        cell{15}{2}    = {r=6,c=1}{c,m},
        cell{2}{3}     = {r=5,c=1}{c,m},
        cell{7}{3}     = {r=4,c=1}{c,m},
        cell{12}{3}    = {r=3,c=1}{c,m},
        cell{15}{3}    = {r=3,c=1}{c,m},
        cell{18}{3}    = {r=3,c=1}{c,m},
    }
...
本研究最终从\autoref{tab:nonparametric-test}中选取了Wilcxon W等级和检验方法，即Mann-Whitney U检验对被试的人口统计学特征、PE风险因子在实验室与对照组上
是否存在分布差异进行了相关性分析。
...
\begin{longtblr}
    ...
    5 & BMI（$kg/m^2$） &  30.2±4.5 &  26.2±3.3 & <0.001\TblrNote{*}\\
...
上均无明显统计意义上的差别（$p$>0.05），但在体重、BMI、收缩压与舒张压的数值上有显著差异（$p$<0.001）。
其中，收缩压与舒张压本身就是PE病发的标致性特点\cite{OAG9,FIGO}，\autoref{tab:factors_res}中结果与被试孕妇的PE病发状态一致，符合预期。
而对同样有显著性差异的体重与BMI，在本论文的前期研究中，已经将其作为协变量进行PE识别的分析研究，结果表明这些参数的贡献度较低，
对识别PE的作用有限\cite{Feng2023}。

综上，本论文将研究的重点聚焦在对被试人员的PPG信号的分析研究上\cite{ChenH2019,Feng2023,FengJiang2018,Jiang2023}。
...
本节介绍了对PPG信号的分析预处理过程，同时说明预处理各过程中使用的具体算法，
并着重介绍了本研究提出的一种可在多平台、多研究背景下应用的新型PPG波形检测算法。
PPG预处理过程包括滤波处理、波形检测、去除基线漂移、样条插值与数据标准化等，如\autoref{fig:process}所示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\linewidth]{pulse_preprocess/process}
    \caption{\label{fig:process}PPG预处理流程示意图}
\end{figure}

\subsection{信号滤波}
数字滤波可从原始信号获取特定频段的成份，是信号处理的常见的处理方法。而在人体电生理信号分析领域，数字滤波更是必不可少的处理步骤。针对人体各电生理信号，此前的诸多学者们提出了多种不同的滤波算法。
...
提出一种基于初筛—复核—决策（screening-checking-deciding，SCD）的新型算法\cite{Jiang2023}，其处理流程如\autoref{fig:detect}所示。
...
初筛阶段不做除波峰、波谷外的其他特征点检测。其他特征点的定位依赖于波形的正确检测，这些检测被推迟到SCD算法检测完成后才进行。
...
不同人群、不同采集设备、不同采集环境下采集得到的PPG信号可能形态上千差万别，与之对应的干扰信号也往往不尽相同。上述标准仍有不足以甄别区分有效信号与无效干扰的可能。
故SCD算法允许根据实际分析需要，支持定义新的复核标准或只使用以上部分复核标准。
...
差分法、阈值法、移动窗法、数字滤波法及由这些方法的组合构成的新方法常用于PPG波形检测过程之中\cite{Chen2019,Chen2020,ChenH2019,Su2007,van2019,van20192}。
...
而SCD算法与DMTW算法检测出的有效PPG波形均在\autoref{fig:scd_detect1}与\autoref{fig:scd_detect2}中用虚线进行了框选，其中黑色虚线表示两种算法对PPG波形的
识别判断完全一致，红色虚线表示两种算法的检测结果出现了分歧。

从整体检测准确率来看，SCD算法较另外两种算法也具有一定的性能优势。统计三种算法在自主实验数据与CBPEDS数据上的错检波形数目，可得到对应算法的整体检测准确率，如\autoref{tab:scd}所示。
可以发现，三种算法中，SCD算法的错检波形数目最少，识别的准确率最高。
...
\subsection{重搏波与切迹检测}
重搏波是PPG最具标志性的波形特征之一，其波峰与切迹的检测是一个完整的PPG检测算法必不可少的定位项之一\cite{Wang2012}。
如\autoref{fig:incisura}所示，当被试出现外周阻力增加、血管壁弹性下降的情况后，尖锐的重搏波切迹（incisura）常会变形甚至丢失，退变成一简单拐点\cite{Wang2012,mmt}。
...
为保证切迹的准确检测，本研究借鉴了王选等于2012年提出的一种基于曲率$K$（curvature）的定位算法进行处理\cite{Wang2012}。
该算法的核心思想是PPG信号在切迹退化成的变形点的邻域内具有最大的曲率值，如\autoref{alg:incisuras_detect}所示。
若点$P(x,y)$为一般曲线函数上一点，则曲线在$P$点的曲率$K$为
\begin{equation}
    \label{equ:curvature}
    K=\frac{|y^{''}|}{{(1+{y^{'}}^2)}^{3/2}}
\end{equation}
其中，$y^{'}$与$y^{''}$分别是曲线函数在$P$点的一阶导数与二阶导数。
...
\subsection{脉搏波的插值处理}
由于此前数据实验采集得到的PPG信号采样率仅为100Hz，为保证后续章节对PPG波形时域特征的计算，
本论文还额外通过基于自然边界条件的三次样条插值算法对PPG数据进行了均匀插值处理\cite{Cheng2008,Timothy2018,Carl2008,ttk2023}。
样条插值使用多个公式来通过所有数据点并得到得到光滑的拟合插值曲线\cite{Timothy2018,Carl2008,ttk2023}。
插值处理后的PPG数据采样率被提高至2000Hz，插值前后效果对比如\autoref{fig:spline}所示。

\begin{figure}[htbp]
    \centering
    \subfigure[插值前采样率为100Hz]{
    \includegraphics[width=7.5cm]{pulse_preprocess/100}
    }
    \quad
    \subfigure[插值后采样率为2000Hz]{
    \includegraphics[width=7.5cm]{pulse_preprocess/2000}
    }
    \caption{\label{fig:spline}插值前后的PPG波形散点图对比}
\end{figure}

\subsection{去除基线漂移}
为消除由于呼吸干扰等原因导致的PPG信号的基线漂移，本研究借助一种基于线性变换的方法进行了处理，这一过程如\autoref{fig:drift}所示。
...
将一个包含$n$个采样点的PPG波形的幅值表示为序列$x_i$，其中$0 \le i \le n-1$。
若始末位置脉搏波幅值不等，则明显两者之间存在一条斜率$k \ne 0$的直线，其中
\begin{equation}
    \label{equ:linek}
...
\subsection{数据标准化}
在上述各项预处理过程完成后，PPG波形在幅值上仍然有很大的个体差异，即使对同一被试对象，其波形在幅值上也会有一定的波动，如\autoref{fig:pe_healthy}所示。
为消除个体差异对特定波形特征计算的影响，需要对PPG波形进行归一化处理。
与去除基线漂移过程类似，只需对PPG波形内所有采样点进行一次线性变换，使单个波形内所有数据点按同一尺度缩放映射到[0,1]区间内。
该过程中的标准化系数也同时被保存记录下来。
...
如\autoref{fig:aix}，AIX指心脏收缩期早期与晚期拐点之间的差值与峰值之比，计算的关键需要先找到表征脉搏波反射波上升冲程（up-stroke）的拐点$P_1$所示\cite{Su2014}。
\begin{equation}
    \label{equ:aix}
...
如\autoref{fig:k}所示，K值是另一种在临床得到广泛应用的基于脉搏波面积变化的形态学特征无量纲参数，最早由罗志昌等\cite{Luo1988,PPGYY}提出
\begin{equation}
    \label{equ:ppgk}
    K=\frac{P_m-P_d}{P_s-P_d}
\end{equation}
其中，$P_m=\frac{1}{T}\int_{0}^{T}P(t)dt$为一个心动周期内脉搏压力$P(t)$的平均值，$P_s$与$P_d$分别为收缩压与舒张压。
罗志昌等\cite{Luo1988,PPGYY}的研究表明，K值依赖于脉搏波的波形动态变化，
同时能够在很大程度上反应血管外周阻力与血管壁硬化程度等生理因素的变化。

如\autoref{fig:ri}所示，RI是由脉搏波舒张峰值或拐点处幅值与收缩谷值之比计算得到的\cite{Su2014,Elgendi2012}
\begin{equation}
    \label{equ:ri}
...
\begin{longtblr}
    ...
    {
        colspec        = {X[0.8,c,m]X[2.7,c,m]X[7,c,m]X[4.5,c,m]},
        hline{1,Z}     = {\thickline},
        hline{2}       = {\thinline},
        rowhead        = 1,
        row{1}         = {font=\headfont},
...
特别地，PWV已经在PE的诸多研究中得到应用\cite{Tomsin2012,Katsipi2014,VivianaIvan2018,Ira2014}。
但由于PWV的测量依赖于在人体不同部位测量脉搏波，且测量多利用压力传感器完成，故\textbf{不适用}单点测量的光电容积脉搏波研究。

\section{小结}
本章首先介绍了临床实验方案，阐述了实验被试人员的筛选标准、实验设备、实验场地与实验流程等信息，初步验证了PE患者与健康孕妇的PPG波形存在形态差异，
完成了论文所需的PPG数据的获取工作。
其次，对被试人员的人口统计学特征进行了统计分析，排除了年龄、孕周等风险因子对孕妇PE病发的影响。借助此前的预研究也排除了体重及BMI的影响，将
研究重点聚焦于PPG之上。
随后，分析了实验获取的PPG信号特点，提出一种新型SCD算法完成了高准确率的波形检测处理，同时也完成了信号滤波、重搏波检测、基线漂移去除、重采样及标准化在内的预处理过程，
完成了后续PPG时域特征提取的数据准备工作。
最后，从时间类参数、幅度类参数、面积类参数与斜率类参数等方面对现有的PPG时域特征进行了介绍，方便后续PPG的形态学特征提取工作的开展。