...
从时长、角度、斜率与面积等维度的提出多种PPG新型时域特征，同时也提出PPG波形特征描述向量及PPG波形间描述特征。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{features/frameworks4}
...
本研究也将PPG波形的上升支与下降支进行线性变换处理时的截距与斜率进行了记录，将这4个参数作为另一类标准化参数，纳入进PPG多维度时域特征集中。
...
在本研究基于PPG特征识别PE病发的研究中，由于全部PPG数据仅来自79名被试，本论文采取了先研究能否通过PPG单个波形进行PE识别，再研究能否
通过被试的全部PPG波形进行PE识别的研究策略。而一名被试通常包含多个PPG波形，故这种处理策略变相的提升了原始数据集的规模，有助于ML模型收敛。
...
\subsection{特征缩放}
一般而言，原始数据的输入特征在数值属性出现较大的差异会导致机器学习模型的性能下降、表现欠佳\cite{Aurélien2018}。为保证这些输入特征能满足特定的机器学习算法的输入要求，
通常还要对这些特征的数值分布进行一定的调整，这也就是特征缩放操作。
