...
此外，从\autoref{fig:no_pe}也可以看到，即使对同一被试而言，不同PPG波形之间也存在这较为明显的时长差异。若不加任何处理，这种差异会直接导致PPG采样序列时域特征集（PPGSTFS）的维数差异。
...
对上节提出的PPG多维度时域特征集（PPGMTFS）与PPG采样序列时域特征集（PPGSTFS）进行了数据集的预处理工作，包括数据集的划分、特征缩放及特征降维。
...
由于本论文使用了PPG多维度时域特征集（PPGMTFS）与PPG采样序列时域特征集（PPGSTFS）这两个数据集，并提出了基于波形（PR）与基于被试（SR）两个具体研究方向，
...
\begin{center}
  ...
  \begin{longtable}{m{1cm}<{\centering}m{1.5cm}<{\centering}m{1.5cm}<{\centering}m{1.5cm}<{\centering}m{1.5cm}<{\centering}m{1.5cm}<{\centering}m{1.5cm}<{\centering}}
    ...
    \label{tab:dataset}\\
        \topline
        \colorhead & & & \multicolumn{2}{c}{\textbf{训练集}} & \multicolumn{2}{c}{\textbf{测试集}}\\
        \colorhead  \multirow{-2}{*}{\textbf{序号}}& \multirow{-2}{*}{\textbf{研究方向}}& \multirow{-2}{*}{\textbf{样本总数}}& \textbf{实验组} & \textbf{对照组} & \textbf{实验组} & \textbf{对照组} \\
        \midline
        \endfirsthead
        \caption[]{(续)}\\
        \midline
        ...
        \bottomline
        \endlastfoot
        \colorrowa 1 & PR  & 7864  & 3746 & 2545 & 937 & 636 \\
        \colorrowc 2 & SR  & 79  & 35 & 28 & 9 & 7 \\           
  \end{longtable}
\end{center}
\vspace{-1em} 
