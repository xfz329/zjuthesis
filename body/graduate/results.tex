...
\section{引言}
本章主要基于PPG时域特征集合完成了PE识别模型的构建与分析工作。
首先，介绍常见的监督学习算法原理与机器学习模型性能评价指标。
其次，为人工扩充数据集，在第四章构建的PPG多维度时域特征集与PPG采样值时域特征集的基础上，分别进行
基于波形与基于被试的PE识别模型的构建分析工作。与此同时，对两个特征集中的特征参数的重要性与贡献度进行评估分析，
进行一定程度上的降维处理工作。最后，比较上述研究中的现象与结果，分析原因并总结归纳本研究的主要结论。
...
本小节介绍了ML中有代表性的决策树(decision tree，DT)算法与随机森林(random forest，RF)算法原理，并介绍了常用于评价模型的性能相关指标。
...
\begin{longtblr}
    ...
    1 & {随机梯\\度下降}        &   6.17   & 87.2\% & 68.9\% & 77.0\% & 75.5\% & 0.876 &  74.8\% \TblrNote{a}& 97.7\% \TblrNote{z}& 84.7\% & 79.0\% \\
    2 & 决策树             &   5.24  & 91.1\% & 83.0\% & 86.9\% & 85.1\% & 0.907 & 93.6\% & 81.4\% & 87.1\% & 85.6\% \\
    3 & K近邻              &   3.08    &  94.7\% \TblrNote{z}&  93.7\% \TblrNote{z} &  94.2\% \TblrNote{z} &  93.1\% \TblrNote{z} &  0.974 \TblrNote{z} & 95.4\% & 93.3\% & 94.3\% \TblrNote{z} & 93.3\% \TblrNote{z} \\
    4 & {高斯朴素\\贝叶斯 }     &   1.22 \TblrNote{a} & 87.9\% & 63.9\% \TblrNote{a} & 74.0\% \TblrNote{a} & 73.2\% \TblrNote{a} & 0.838 \TblrNote{a} & 90.1\% &  65.0\% \TblrNote{a} & 75.5\% \TblrNote{a} & 74.9\% \TblrNote{a} \\
    5 & 逻辑回归           &   203.00 \TblrNote{z} & 90.2\% & 90.7\% & 90.4\% & 88.6\% & 0.950 & 93.3\% & 93.0\% & 93.1\% & 91.8\% \\
    6 & {线性支持\\向量机}     &   47.22  & 79.7\% \TblrNote{a}& 93.2\% & 86.0\% & 81.9\% & 0.917 & 95.9\% \TblrNote{z} & 81.9\% & 88.3\% & 87.1\% \\
...
由于在进行超参数优化时是以各ML算法模型在测试集上的AUC数值为衡量标准，故图\autoref{fig:cmdl1}中各算法在优化后AUC均出现了一定的增长；
...
\begin{longtblr}
    ...
    1	&   CVALF\_9 \TblrNote{f}	& 4.6\%	&   13   &	STDZ\_1	    &   1.5\%   &	25	&   CVRF\_1	\TblrNote{f}    &   0.9\%   \\
    2	&   LVRF\_9	\TblrNote{f}& 3.4\%	&   14   &	CVALF\_8 \TblrNote{f}	    &   1.5\%   &	26	&   CVAAF\_2 \TblrNote{f}    &   0.9\%    \\
    3	&   SVD\_10	& 3.0\%	&   15   &	SVD\_9	    &   1.4\%   &	27	&   SVAR\_8	\TblrNote{r}    &   0.8\%   \\
    4	&   SVAF\_10 \TblrNote{f}	& 2.5\%	&   16   &	SVAAR\_10  \TblrNote{r}  &   1.4\%   &	28	&   SVAF\_2	 \TblrNote{f}   &   0.8\%   \\
    5	&   LVALF\_7 \TblrNote{f}	& 2.4\%	&   17   &	CVALF\_1 \TblrNote{f}	    &   1.4\%   &	29	&   CVRR\_11 \TblrNote{r}    &   0.8\%   \\
    6	&   SVAT\_10	& 2.3\%	&   18   &	SVAAR\_8 \TblrNote{r}	    &   1.3\%   &	30	&   LVRF\_1	\TblrNote{f}    &   0.7\%   \\
    7	&   CVRF\_11 \TblrNote{f}	& 2.1\%	&   19   &	SVD\_8	    &   1.1\%   &	31	&   SVSR\_7	\TblrNote{r}    &   0.7\%   \\
    8	&   SVAR\_10 \TblrNote{r}	& 1.8\%	&   20   &	SVAR\_9	\TblrNote{r}    &   1.0\%   &	32	&   CVRF\_3	\TblrNote{f}    &   0.7\%   \\
    9	&   STDZ\_3	& 1.6\%	&   21   &	SVAF\_9	\TblrNote{f}    &   1.0\%   &	33	&   CVALR\_10  \TblrNote{r}  &   0.7\%   \\
    10	&   LVRF\_8	\TblrNote{f}& 1.6\%	&   22   &	CVAAF\_1 \TblrNote{f}	    &   1.0\%   &	34	&   CVALF\_7 \TblrNote{f}    &   0.7\%   \\
    11	&   LVALF\_6 \TblrNote{f}	& 1.6\%	&   23   &	LVLF\_8	\TblrNote{f}    &   0.9\%   &	35	&   CVAAR\_5 \TblrNote{r}    &   0.7\%    \\
    12	&   CVD\_11	& 1.6\%	&   24   &	SVAT\_8	    &   0.9\%   &	36	&   LVRF\_2	\TblrNote{f}    &   0.6\%   \\
\end{longtblr}
\vspace{2em}
...
\begin{longtblr}
    ...
    序号 & 被试 & {PE\\状态} & {波形\\总数} & K近邻 & & 决策树 & & 随机森林 & \\
    序号 & 被试 & {PE\\状态} & {波形\\总数} & 预测数目 & 预测比例 & 预测数目 & 预测比例 & 预测数目 & 预测比例 \\    
    1  &     cmf       & 0           & 88      & {66 \\ 59}         & {75.0\% \\ 67.0\%}     & {75 \\ 60}         & {85.2\% \TblrNote{p} \\ 68.2\% \TblrNote{p}}   & {61 \\ 49}         & {69.3\% \\ 55.7\%}              \\
    2  &     lxx       & 0           & 63      & { 32 \\ 35 }         & {50.8\% \\ 55.6\%}    & {50 \\ 38}         & {79.4\% \TblrNote{p}\\ 60.3\% \TblrNote{p}}    & {37 \\ 42}         & {58.7\% \\ 66.7\%}              \\
    3  &     shs       & 0           & 112     & { 32 \TblrNote{n} \\ 58 \TblrNote{n}}        & {28.6\% \TblrNote{p} \\ 51.8\% \TblrNote{p}}    & {102 \\ 88}        & {91.1\% \\ 78.6\%}     & {57 \\ 47}         & {50.1\% \\ 42.0\%}               \\
    4  &     sxh       & 0           & 95      & { 31 \\ 24 }        & {32.6\% \\ 25.3\%}     & {52 \TblrNote{n} \\ 27 \TblrNote{n} }        & {54.7\% \TblrNote{p} \\ 28.4\% \TblrNote{p} }    & {39 \\ 21}         & {41.1\% \TblrNote{p} \\ 22.1\% \TblrNote{p}}               \\
    ...
    8  &     gmn       & 1           & 139     & { 51 \TblrNote{n} \\ 82\TblrNote{n}}       & {36.7\% \TblrNote{p}\\ 59.0\% \TblrNote{p} }    & {131 \\ 131}        & {94.2\% \\94.2\%}    & {107 \\ 112}        & {77.0\% \\ 80.6\%}                \\
    ...
    11 &     xjf       & 1           & 106     & { 13 \TblrNote{n} \\ 71 \TblrNote{n}}       & {12.3\% \TblrNote{p} \\ 67.0\% \TblrNote{p} }    & {29 \TblrNote{n} \\ 76 \TblrNote{n}}        & {27.4\% \TblrNote{p} \\ 71.7\% \TblrNote{p}}   & {36 \TblrNote{n} \\ 72 \TblrNote{n}}        & {34.0\% \TblrNote{p} \\ 67.9\% \TblrNote{p}}                   \\
    12 &     ywy       & 1           & 111     & { 111 \TblrNote{n}\\ 42 \TblrNote{n}}       & {100.0\% \TblrNote{p} \\ 37.8\% \TblrNote{p}}     & {110 \TblrNote{n}\\ 65 \TblrNote{n}}       & {99.1\% \TblrNote{p} \\ 58.6\% \TblrNote{p}}   & {110 \TblrNote{n} \\ 66 \TblrNote{n}}       & {99.1\% \TblrNote{p} \\ 59.5\% \TblrNote{p}}                   \\
    ...
    15 &     zl        & 1           & 152     & { 114 \TblrNote{n} \\ 140 \TblrNote{n} }       & {75.0\% \TblrNote{p}\\ 92.1\% \TblrNote{p}}    & {112 \TblrNote{n} \\ 139 \TblrNote{n}}        & {73.7\% \TblrNote{p} \\ 91.4\% \TblrNote{p} }    & {136 \\ 143}        & {89.5\% \\ 94.1\%}                \\
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=7cm]{results/roc_31}
    }
    \quad
    \subfigure[\label{fig:roc_32}重采样策略下模型得到的PE预测比例的ROC示意图]{
    \includegraphics[width=7cm]{results/roc_32}
    }
...
\section{小结}
本章首先阐述了介绍了常见的机器学习算法与模型性能评价指标。其次，在PPG多维度时域特征集上，进行了数据集人工扩充，分别将被试PPG数据的单个波形与全波波形分别作为最小分析单位，
进行了PE识别模型的探究。在完成多算法模型筛选、模型超参数优化之后，决策树、K近邻与随机森林三种算法构建的PE识别模型性能最佳。
随后，通过随机森林算法对PPG多维度时域特征集进行了特征贡献度分析与降维处理等工作，精简了原始数据集，得到了与PE相关性较强的特征主要集中在下降支，且
集中在下降支的前端与尾端。另一方面，本章也在PPG采样序列时域特征集上进行了类似的处理工作，得到了较为相似的结果。
最后，对比两个特征集上不同算法的性能表明，本研究提出的PPG多维度时域特征集具有一定的优势。

上述结果表明，本论文构建的PPG多维度时域特征集与PPG采样序列时域特征集设计合理，具有能表征PE的关键特征。其中，PPG多维度时域特征集较PPG采样序列时域特征集更有优势。
在这两个特征集上，使用决策树、K近邻与随机森林三种算法能够构建较好的PE识别模型，其中随机森林算法构建的模型性能最佳。
因此，基于PPG的时域特征集，通过ML算法构建PE识别模型具有一定的可行性。