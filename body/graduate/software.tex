...
软件系统对滤波处理思路与数据导入类似，也在这一过程中采取了模板方法（Template Method）的设计模式\cite{Enrich2018}。
...
另一方面，考虑到Android客户端的实际使用场景，上文介绍的PC客户端特殊功能目前未在Android客户端下实现。
...
模块对数据合并的处理流程如所示。依次读取需要合并的数据特征文件，确定生成矩阵的标题栏，记录下包括软件版本号、原始PPG数据文件名、数据患者名、PE状态等基本信息，
...
本模块对数据划分的流程如所示。一个单独的数据集管理类负责返回所有模型训练、评估的训练集与测试集，而在该类内部，需要返回数据时，函数都会先去检查硬盘上是否已有划分好的数据集。
...
对机器学习算法的具体封装过程如所示。MyClasssifer是封装之后的类名，可直接用Sklearn工具包中的具体分类算法的实例初始化该类。除此之外，该类只有三个公共函数接口，
一个为设定MyClasssifer所使用的分类算法的实例，该实例也必须为Sklearn工具包中有效实例；
一个为设定模型训练所需的训练集与测试集数据函数loadData；最后一个为开始训练并评估的fitPredict函数，该函数封装了机器学习算法在训练集与测试集上的全部处理操作过程。
在具体使用时，使用封装后MyClasssifer类评估不同的机器学习方法在数据集上的表现时，可实现代码的最小化，简化了模型训练过程。

另外，在上述过程中，为使训练好的机器学习模型供后续上传至云服务器使用，相关结果也必须及时保存。
这一过程也被封装在了MyClasssifer的fitPredict函数中，生成的最终模型与训练模型的超参数也被同时保存下来。
除随机森林算法构建的模型因为算法本身的随机性导致训练模型的不可控外，只要给定相同的数据集，选中固定的算法，在给予相同的超参数的条件下，就可以复现出完全相同的最终识别模型。
...
相关功能的配置，具体配置过程可参见官网，这里不再赘述。\autoref{fig:clflog}给出了模型训练生成模型最终的日志记录示意。
\begin{figure}[htbp]
    \centering
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.7\linewidth]{/software/Retrofit}
...
机器学习模型上传程序，如所示。而各机器学习模型在上传时，本质上仍使用了HTTP协议的POST方法。该POST方法在与云服务器端进行通讯时携带的参数及含义如\autoref{tab:post_paras2}所示，
...
云服务器端的程序处理流程如\autoref{fig:django2}所示。此前介绍过的\autoref{tab:post_paras}中的跨平台客户端的数据请求与\autoref{tab:post_paras2}中的
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.8\linewidth]{software/django}
    \caption{\label{fig:django2}Django框架结构示意\cite{djangosq}}
\end{figure}

以上介绍的处理流程实际上对应着\autoref{tab:django_func}中的视图，即业务逻辑层。而对于\autoref{tab:django_func}中的模板表现层而言，目前云服务器端程序对网络请求的处理较为简单，
只以纯文字的形式返回内容，没有进行专门的网页UI设计，故这部分不做过多介绍。而\autoref{tab:django_func}中的模型涉及了数据库的处理操作，这部分的程序设计会在后续进行介绍。
