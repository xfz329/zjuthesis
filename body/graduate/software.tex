...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.75\linewidth]{software/scas}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.5\linewidth]{software/read}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.8\linewidth]{software/singleton}
...
软件系统最终的导出数据示例如\autoref{fig:json}所示。可以看到，除基本数据特征外，
与数据相关的文件信息及孕妇人口统计学信息也同时被保存。
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.85\linewidth]{software/pc}
...
PC端软件正常的处理流程如\autoref{fig:pc_process}所示。在选中需要分析的PPG文件后，软件系统会自动调用SCD算法的进行波形初筛，并将筛选结果在波形显示区域进行标注。
...
输出到不同日志文件中，方便系统开发与调试维护，如\autoref{fig:logs}所示。一般而言，不同的日志文件记录的信息重要性递减，但输出的详细程度递增。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{software/logs}
...
这一过程中的使用的配置文件范例如\autoref{fig:configure}所示。
% ，而\autoref{tab:configure}也对\autoref{fig:configure}中内容进行了进一步的解释与说明
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\linewidth]{software/configure}
...
该过程即为第四章的脉搏波多维度时域特征集（photoplethysmographic multidimensional
...
在PE识别模型的构建过程中，由于涉及多个模型的比较，需要不断重复上述操作。如果不对该过程进行设计，该过程不但繁琐且极易疏漏。
另一方面，由于PE识别模型均是借助Python下Sklearn工具包完成的，而Sklearn本身在设计过程中均遵循了统一规范，不同的机器学习算法均提供了相同的API函数。
而Python本身是一种动态类型的编程语言，并不再编译时确定对象类型，而是将类型检查延迟至运行时。换言之，Python不会检测变量的具体类型，而是关注变量
是否具有特定的属性与方法。基于以上背景，软件系统对PE识别模型的构建过程额外进行了封装，在实现代码的最小化的同时，也避免了算法模型训练过程的代码遗漏等问题，同时也简化了模型训练过程。
...
截止目前，客户端程序通过POST方法向云服务器端提交数据的通讯过程中涉及到的所有参数如\autoref{tab:post_paras}所示，其中文件类数据在表格中用粉红色底色
进行了突出显示。
\begin{longtblr}
    [
        theme                   = {zju},
...
在上述过程中，为实现与云服务器端进行正常通讯，PC客户端选择了HttpClient这一组件进行网络功能的开发，
Android客户端选择了Retrofit这一组件进行网络功能的开发，如\autoref{tab:platform}所示。
...
机器学习模型上传程序，如\autoref{fig:upload_model}所示。模型在上传时需将包括使用的算法、训练集、版本号等信息一并提交。
%而各机器学习模型在上传时，本质上仍使用了HTTP协议的POST方法。%除\autoref{fig:upload_model}中已展示的外，
% 该POST方法在与云服务器端进行通讯时携带的全部参数及含义如\autoref{tab:post_paras2}所示，
% 其中文件类数据在表格中用粉红色底色进行了突出显示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=.65\linewidth]{software/upload_model}
...
由于本研究涉及的所有机器学习模型均是通过Python下的Sklearn包生成，为方便云服务器端调用，软件系统最终选取了同样基于Python的开源通用Web框架Django进行相关云服务器端的功能开发。
如\autoref{fig:django}所示\cite{django,djangosq}，Django按模型-模版-视图（Model-Template-View，MTV）设计模式的进行了整体结构设计，MTV各自的职责与功能
...
云服务器端的程序处理流程实际上对应着\autoref{tab:django_func}中的视图，即业务逻辑层。当云服务器端接收到客户端的数据请求或模型上传请求时，云服务器端程序会依据请求对应的不同
URL路由地址，选择对应的处理流程，这一过程如\autoref{fig:url}所示。
% ，此前介绍过的\autoref{tab:post_paras}中的跨平台客户端的数据请求与\autoref{tab:post_paras2}中的
% 模型上传请求分别对应云服务器端的两个路由地址，即两个URL地址。对模型上传请求而言，，会在确认当前并无重复的算法及其版本后，将新的
...
请求响应是云服务器端程序完成\autoref{fig:url}中的处理后对客户端的答复信息，对应着\autoref{tab:django_func}中的模板表现层。一般而言，请求响应可包含文本、音视频文件与HTML网页文件等多种形式的内容。Django网络框架中默认支持的请求响应包括两种，一种为普遍通用的HttpResponse，可在其中传达文本消息及文件等；另一种为功能更专一化的JSONResponse，可在其中按JSON数据格式传递键值对。考虑到客户端程序解析与后续调整的方便，云服务器端程序的请求响应采用了JSONResponse的格式，传递内容仅为文本字符，暂未进行专门的网页UI设计。

以\autoref{fig:url}中数据请求的响应为例，云服务器端返回的具体内容如\autoref{tab:json_response}所示。
由于数据请求可能同时请求调用多个训练好的模型，故其请求响应中可能返回多个模型的分析结果。其中，返回模型的总数由model\_num记录；
...
数据库（DataBase， DB）技术是现代互联网数据管理的基本方法，可有效组织、存储和管理大量数据。其中，数据一词指数据库可以储存多种信息，这些信息一般被称为字段。
而为了从包含多条数据记录的集合中获取特定记录，还必须从这些字段中定义主键（primary key）。
主键通常可以是一个字段或多个字段的组合，其值能唯一地标识表中的每一行，在与其他表的外键关联以及数据记录的修改与删除中具有重要作用。
% 而关系型数据库是对存储的格式能够直观地反映实体间的关系的一类数据库的统称。关系型数据库和常见的表格比较相似，数据库的表与表之间是有很多复杂的关联关系。

另一方面，如\autoref{fig:url}所示，云服务器端程序在处理客户端请求时，需要对请求中包含的大量参数信息进行处理与储存。
而这些请求中的参数种类常常会随着软件系统的迭代而发生较大的改变，比如数据信号种类的变更、PPG特征的增改等。这也要求软件系统使用数据库进行数据的基本管理时，还必须同时
进行一定的兼容性设计。
...
在以上背景下，软件系统选择了目前流行的主流关系数据库之一的MySQL进行相关数据的存储。MySQL遵循结构化查询语言（Structured Query Language，SQL）标准，支持查询、新增、更新、删除、求和、排序等基本操作。与此同时，软件系统一共设计了五张数据表（Table）来解决上述问题。%这些数据表之间的相互关联关系如\autoref{fig:data_table_relationship}所示。
其中，模型的存储过程较为简单，只需一张数据表；而生理信号数据及其衍生特征的存储较为复杂，需要四张数据表级联并相互配合。
%需要注意的是，为简明扼要，\autoref{fig:data_table_relationship}中仅显示了各数据表的重要的外部键，
这些数据表的详细设计可参见\autoref{tab:data_table_record}至\autoref{tab:data_table_model}，其中数据表的主键与外部键用粉色进行了突出显示。
\begin{longtblr}
    [
        theme                   = {zju},
...
Record数据表是存储特征时使用到的最为底层的数据表，主要用来记录存储数据请求中的最原始数据文件，而id字段为其主键，如\autoref{tab:data_table_record}所示。
...
%     \centering
%     \subfigure[PC客户端]{
%     \includegraphics[width=10cm]{software/pc_demo}
%     }
%     \quad
%     \subfigure[Android客户端]{
%     \includegraphics[width=10cm]{software/android_demo}
%     }
%     \caption{\label{fig:pc_android_demos}不同客户端对被试dmq的PPG数据检测结果}
% \end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{software/android_demo}
    \caption{\label{fig:android_demo}Android客户端对被试dmq的PPG数据检测结果}
\end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{software/pc_demo}
    \caption{\label{fig:pc_demo}PC客户端对被试dmq的PPG数据检测结果}
\end{figure}

为测试验证两个平台下的客户端软件，分别用客户端软件选中相同的PPG数据，两个平台下的检波效果完全一致，如\autoref{fig:android_demo}与\autoref{fig:pc_demo}所示。
...
\autoref{fig:pc_response}与\autoref{fig:android_response}分别对应着被试cmf的原始数据在PC客户端与Android客户端的响应结果，而
...
首先，详细分析了软件系统的研发需求。其次，介绍了软件系统的整体性多模块化设计。
随后，针对各个模块的设计需求，结合实际开发中遇见的问题，对各模块的实现工作进行了介绍。最后，对各模块进行了一定的测试。
