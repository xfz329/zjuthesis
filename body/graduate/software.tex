...
一个为设定模型训练所需的训练集与测试集数据函数$loadData()$；最后一个为开始训练并评估的$fit\_predict()$函数，该函数封装了机器学习算法在训练集与测试集上的全部处理操作过程。
在具体使用时，使用封装后MyClasssifer类评估不同的机器学习方法在数据集上的表现时，可实现代码的最小化，简化了模型训练过程。

另外，在上述过程中，为使训练好的机器学习模型供后续上传至云服务器使用，相关结果也必须及时保存。
这一过程也被封装在了MyClasssifer的$fit\_predict()$函数中，生成的最终模型与训练模型的超参数也被同时保存下来。
